 ...
 */
function notifyFirstWaiter<K, T>(waiters: Map<K, Array<(data: T) => void>>, key: K, data: T): void {
  const callbacks = waiters.get(key);
 ...
 */
function notifyAllWaiters<K, T>(waiters: Map<K, Array<(data: T) => void>>, key: K, data: T): void {
  const callbacks = waiters.get(key);
  if (!callbacks) return;
...
function invokeCallbacks<T>(callbacks: Array<(data: T) => void>, data: T): void {
